...
public class Questions {
	...
	public String multiChar(String input) {
		...
		for (int i = 0; i < input.length(); i++) {
			str = str + String.valueOf(input.charAt(i)) + String.valueOf(input.charAt(i))
	...
	public String sandwichFilling(String sandwich) {
		...
		if (sandwich.contains("bread")) {
			int number = sandwich.indexOf("bread");
			sandwich = sandwich.substring(number + 5, sandwich.length());

			if (sandwich.contains("bread")) {
				int number2 = sandwich.indexOf("bread");
				sandwich = sandwich.substring(0, number2);

				String str = "";
				for (int i = sandwich.length() - 1; i >= 0; i--) {
					str = str + sandwich.charAt(i);
